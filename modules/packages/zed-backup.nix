{ pkgs, interval, maxBackups, backupDir }:

pkgs.writeScriptBin "zed-backup" ''
  ...
  BACKUP_INTERVAL = ${toString interval}
  MAX_BACKUPS_TO_KEEP = ${if maxBackups == null then "None" else toString maxBackups}
  ...
  def main():
      backup_root = Path("${backupDir}").expanduser()
