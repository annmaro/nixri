<div align="center">
  <img src="img/nixos-logo.png" width="100" alt="NixOS Logo" />
  <h1>My NixOS Configuration</h1>
  <img src="img/catppuccin-macchiato.png" width="600" alt="Catppuccin Macchiato Color Palette" />
</div>
...
- DMS or Noctalia desktop shell selection.
...
- Declarative browser, terminal, editor, CLI, media, and development tools.
...
> [!IMPORTANT]
> Use `nixos-rebuild boot` rather than `nixos-rebuild switch` for this configuration. The SpoofDPI and host-blocking services can produce errors when activated with `switch`. `boot` builds and activates the configuration for the next reboot without replacing the currently running system generation.
...
Reboot after the build to start the new generation:

```bash
systemctl reboot
```

Use `nixos-rebuild switch` only if you have explicitly confirmed that the SpoofDPI and host-blocking services are not enabled or affected.
...
They are serialized into the generated Niri configuration by the wrapper. Press `Ctrl+Shift+K` to display the available keybindings through the configured Rofi helper.

## Credits and inspiration

| Credit | Use |
| --- | --- |
| Rofi-adi109x | Custom launcher and power menu inspiration |
